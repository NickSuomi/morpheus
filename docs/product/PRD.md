...
- Valid transitions are `agent:ready` -> `agent:preparing`, `agent:ready` -> `agent:failed` when duplicate source identity is detected during import, `agent:preparing` -> `agent:prepared`, `agent:preparing` -> `agent:blocked`, `agent:preparing` -> `agent:failed`, `agent:prepared` -> `agent:running`, `agent:prepared` -> `agent:failed` when implementation setup fails before the implementer runs or duplicate source identity is detected during import, `agent:running` -> `agent:reviewing`, `agent:running` -> `agent:blocked`, `agent:running` -> `agent:failed` when implementation fails or duplicate source identity is detected during import, `agent:reviewing` -> `agent:review-candidate`, `agent:reviewing` -> `agent:blocked`, `agent:reviewing` -> `agent:failed`, `agent:review-candidate` -> `agent:failed` when a previously passing or pending MR gate later fails, `agent:blocked` -> `agent:ready` by human/operator requeue, and `agent:failed` -> `agent:ready` by human/operator requeue.
- `agent:review-candidate` has no Morpheus auto-merge transition.
- Events are `DuplicateImportDetected`, `StartPreparation`, `PreparationReady`, `PreparationBlocked`, `PreparationFailed`, `StartImplementation`, `ImplementationReadyForReview`, `ImplementationBlocked`, `ImplementationFailed`, `StartReview`, `ReviewPassed`, `ReviewBlocked`, `ReviewFailed`, `HumanRequeued`, and `HumanRetryFailed`.

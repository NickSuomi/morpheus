...
Morpheus turns an issue into inspectable evidence before a human reviews the
merge. If preparation finds weak intent, blockers, or unresolved decisions, the
run stops before implementation.
...
```sh
scripts/package-release.sh --version 0.1.24 --only-os darwin --only-arch arm64
```

Install from local artifact by overriding URL/checksum:

```sh
MORPHEUS_RELEASE_URL="file:///path/to/morpheus-0.1.24-darwin-arm64.tar.gz" \

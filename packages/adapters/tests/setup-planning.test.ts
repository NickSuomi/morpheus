...
describe("setup planning", () => {
  ...
  it("normalizes URL-like GitLab project answers into project paths", () => {
    const plan = planMorpheusSetup({
      ...
      existing: { files: [] },
      answers: {
        gitlabProject: "gitlab.example.com/example-group/example-app.git",
      },
    });
    ...
    expect(plan.prompts).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: "gitlabProject",
          value: "example-group/example-app",
          validation: { status: "valid" },
        }),
      ]),
    );
    ...
    expect(plan.configMutation.nextConfig.gitlab).toMatchObject({
      project: "example-group/example-app",
      targetBranch: "dev-6",
    });
  });
  ...
  it("detects the checked-out branch as the setup target before origin HEAD", () => {
    withTempDir((dir) => {
      ...
      execFileSync(
        "git",
        ["remote", "add", "origin", "git@gitlab.example.com:example-group/example-app.git"],
        {
          cwd: dir,
          stdio: "ignore",
        },
      ...
      expect(input.detected?.gitlabProject).toBe("example-group/example-app");
    });
  });

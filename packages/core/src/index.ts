...
export const renderReviewArtifact = ({
  ...
  [
    ...
    "## Key Interfaces",
    "",
    ...bulletList(contract.keyInterfaces, "No key interfaces recorded."),
    "",
    "## Acceptance Criteria",
    "",
    ...bulletList(contract.acceptanceCriteria, "No acceptance criteria recorded."),
    "",
    "## Out Of Scope",
    "",
    ...bulletList(contract.outOfScope, "No out-of-scope items recorded."),
    "",
    "## Verification Plan",
    "",
    ...bulletList(contract.verificationPlan, "No verification plan recorded."),
    "",
    "## Blockers",
    "",
    `Blocked by: ${contract.blockedBy}`,
    "",
    "## HITL Decisions",
    "",
    `HITL decisions: ${contract.hitlDecisions}`,
    "",
...
const isAgentState = (label: string): label is AgentState => agentStateSet.has(label);
...
const normalizeLaneCapacities = (
  ...
  preparation: normalizeCapacity(config.preparation, defaultLaneCapacities.preparation),
  implementation: normalizeCapacity(config.implementation, defaultLaneCapacities.implementation),
...
const issueDate = (issue: LaneSchedulerIssue): string => issue.createdAt ?? issue.updatedAt ?? "";

const compareLaneIssues = (left: LaneSchedulerIssue, right: LaneSchedulerIssue): number =>
